...
##
##
##Функция редактирования дефектного акта (ДА)
##
##
def edit_DA_excel():
    ...
    for row in range(1, last_row + 1):
        ...
        for col in range(1, last_col + 1):
            ...
            # ищем строку Главным инженером
            if "Принял:" in text and "Главный инженер" in text:
                esipov_row = row
    ...
    # оставляем строку с главным инженером и строку под ней
    if esipov_row:
        if esipov_row in rows_to_delete:
            rows_to_delete.remove(esipov_row)
 
        if esipov_row + 1 in rows_to_delete:
            rows_to_delete.remove(esipov_row + 1)
...
def edit_ks2_excel():
    ...
    for row in range(1, last_row + 1):
        ...
        if "Принял:" in row_text and "Главный инженер" in row_text:
            engeneer_row = row
 
    # --- удаляем между ВСЕГО и Главный инженер ---
 
    if total_row and engeneer_row:
        for r in range(engeneer_row - 2, total_row, -1):
            ws.Rows(r).Delete()
...
def edit_ks3(wb, base_name, address, _first_day, _last_day, sum_value):
    ...
    # --- адрес стройки ---
    ws.Range("A10").Value = f"Стройка - МКД по адресу: г. Москва, ул. {address}"
...
##
## Функция получения даты из документа
##
...
##
##
## Функция сохранения дефектного акта (ДА)
##
##
...
print(f"ПОСЛЕДНИЙ ДЕНЬ МЕСЯЦА---- {Last_day_date}")
print(f"ПЕРВЫЙ ДЕНЬ МЕСЯЦА---- {First_day_date}")
